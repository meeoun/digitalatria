<template>
  <div v-if="$auth.loggedIn">
    ...
    <el-dialog :visible.sync="visible" title="Replying To" >
      ...
      <el-form ref="comment-form" :rules="rules" :model="form" label-width="120px" v-loading="loading" element-loading-text="Loading..." element-loading-spinner="el-icon-loading">
        <el-form-item label="Type Reply"  prop="body">
          <el-input type="textarea" v-model="form.body"></el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" class="el-button--default" @click="onSubmit('comment-form')"><p style="color: white">Create</p></el-button>
...
export default {
  ...
  data () {
    ...
    async submit(){
      this.loading = true
      await APIAuthService.storeReply(this.post,this.comment.id,this.$auth.getToken('local'),{body: this.form.body}).then( resp =>
        {
          let reply = resp.data.data
          this.loading = false
          this.visible = false;
          this.$store.commit('posts/ADD_REPLY',reply)
